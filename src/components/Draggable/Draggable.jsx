    ...
    <div >
      ...
      <div className={''}>
        ...
        <table className={'flex flex-col gap-2 overflow-y-auto webkit-scrollbar webkit-scrollbar-track:[#423e3e1a] webkit-scrollbar-thumb'}>
          ...
          {status === 'fulfilled' && bookings?.map(
            (res, i) => (
              <tr key={i} className={'border-b-[1px] border-[#423e3e1a] py-[13px] grid grid-cols-4 '}>
                <td className={'text-[#404040] text-[14px] leading-[19px] font-normal col-span-2'}>
                {res.organizer_name}</td>
                <td className={'text-[#AEAEAE] text-[14px] leading-[19px] font-normal'}>
          ...
          {status === 'fulfilled' && bookings?.length === 0 && <div>нет данных</div>}
        </table>
